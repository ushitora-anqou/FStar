...
let pat :
  'Auu____251 . 'Auu____251 -> 'Auu____251 FStar_Syntax_Syntax.withinfo_t =
  ...
  fun t1  ->
    fun t2  ->
      ...
      let zbranch =
        ...
        let uu____545 = FStar_Tests_Util.nm x1  in
      ...
      let sbranch =
        ...
        let uu____639 =
          ...
          let uu____645 =
            let uu____648 =
              let uu____649 = FStar_Tests_Util.nm x1  in pred_nat uu____649
               in
            let uu____652 =
      ...
      let lb =
        ...
        let uu____671 =
          let uu____674 =
            let uu____675 =
              let uu____676 = b x1  in
              let uu____683 = let uu____692 = b y1  in [uu____692]  in
              uu____676 :: uu____683  in
            let uu____717 =
              let uu____720 = FStar_Tests_Util.nm y1  in
...
let (encode_nat : Prims.int -> FStar_Syntax_Syntax.term) =
  fun n1  ->
    let rec aux out n2 =
      ...
      then out
      else (let uu____790 = snat out  in aux uu____790 (n2 - Prims.int_one))
       in
    aux znat n1
      ...
      'Auu____3900 ->
        FStar_Syntax_Syntax.term' FStar_Syntax_Syntax.syntax ->
          (FStar_TypeChecker_Env.env ->
             'Auu____3900 -> FStar_Syntax_Syntax.term)
            -> unit
  =
  fun i  ->
    fun r  ->
        ...
        fun normalizer  ->
          ...
          (let tcenv = FStar_Tests_Pars.init ()  in
           (let uu____3943 = FStar_Main.process_args ()  in
            FStar_All.pipe_right uu____3943 (fun a1  -> ()));
           (let x1 = normalizer tcenv r  in
            ...
            FStar_Options.set_option "print_implicits"
              (FStar_Options.Bool true);
            (let uu____3966 =
               let uu____3968 = FStar_Syntax_Util.unascribe x1  in
               FStar_Tests_Util.term_eq uu____3968 expected  in
             FStar_Tests_Util.always i uu____3966)))
...
let (run_interpreter_with_time :
  ...
  fun i  ->
    fun r  ->
      fun expected  ->
        let interp uu____4047 = run_interpreter i r expected  in
        let uu____4048 =
          let uu____4049 = FStar_Util.return_execution_time interp  in
...
let (run_nbe_with_time :
  ...
  fun i  ->
    fun r  ->
      fun expected  ->
        let nbe1 uu____4087 = run_nbe i r expected  in
        let uu____4088 =
          let uu____4089 = FStar_Util.return_execution_time nbe1  in
          FStar_Pervasives_Native.snd uu____4089  in
        (i, uu____4088)
...
let run_tests :
  'Auu____4100 .
    (Prims.int ->
       FStar_Syntax_Syntax.term' FStar_Syntax_Syntax.syntax ->
         FStar_Syntax_Syntax.term' FStar_Syntax_Syntax.syntax -> 'Auu____4100)
      -> 'Auu____4100 Prims.list
  =
  fun run1  ->
    ...
    (let l =
       FStar_List.map
         (fun uu___0_4152  ->
            match uu___0_4152 with | (no,test,res) -> run1 no test res) tests
        in
...
let (run_both_with_time :
  ...
  fun i  ->
    fun r  ->
      fun expected  ->
        let nbe1 uu____4289 = run_nbe i r expected  in
        let norm1 uu____4295 = run_interpreter i r expected  in
...
let (compare : unit -> unit) =
  fun uu____4308  ->
    ...
    (let uu____4311 =
       let uu____4312 = encode (Prims.of_int (1000))  in
       let uu____4314 =
         let uu____4317 = FStar_Tests_Util.nm FStar_Tests_Util.x  in
         let uu____4318 = FStar_Tests_Util.nm FStar_Tests_Util.x  in
         minus uu____4317 uu____4318  in
       let_ FStar_Tests_Util.x uu____4312 uu____4314  in
     run_both_with_time (Prims.of_int (14)) uu____4311 z)
  ...
  fun l_int  ->
    fun l_nbe  ->
      ...
      FStar_List.iter2
        (fun res1  ->
           fun res2  ->
             let uu____4394 = res1  in
             match uu____4394 with
             | (t1,time_int) ->
                 let uu____4404 = res2  in
                 (match uu____4404 with
                  | (t2,time_nbe) ->
                      if t1 = t2
                      then
                        let uu____4416 = FStar_Util.string_of_int t1  in
                        FStar_Util.print3 "Test %s\nNBE %s\nInterpreter %s\n"
                          uu____4416 (FStar_Util.string_of_float time_nbe)
...
let (run_all : unit -> unit) =
  fun uu____4427  ->
    (let uu____4429 = FStar_Syntax_Print.term_to_string znat  in
     FStar_Util.print1 "%s" uu____4429);
