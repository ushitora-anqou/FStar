...
let mksyn_range (p1:FSharp.Text.Lexing.Position) p2 =
    mk_range p1.FileName (pos_of_lexpos p1) (pos_of_lexpos p2)

let getLexerRange (lexbuf:FSharp.Text.Lexing.LexBuffer<char>) = (* UnicodeLexing.Lexbuf) = *)
...
let lhs (parseState: FSharp.Text.Parsing.IParseState) =
...
let rhspos (parseState: FSharp.Text.Parsing.IParseState) n =
...
let rhs2 (parseState: FSharp.Text.Parsing.IParseState) n m =
...
let rhs (parseState: FSharp.Text.Parsing.IParseState) n =
...
let lexeme (lexbuf : FSharp.Text.Lexing.LexBuffer<char> (*UnicodeLexing.Lexbuf*)) =
    FSharp.Text.Lexing.LexBuffer<char> (*UnicodeLexing.Lexbuf*).LexemeString(lexbuf)
let ulexeme lexbuf = lexeme lexbuf

let adjust_lexbuf_start_pos (lexbuf:FSharp.Text.Lexing.LexBuffer<char> (*UnicodeLexing.Lexbuf*)) p =  lexbuf.StartPos <- p
